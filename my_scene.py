...
import time
# นำเข้าโมดูล random เพื่อสุ่มตำแหน่งดาวและอุกกาบาต
import random
...
def main():
    ...
    meteors = create_meteors(canvas, NUM_METEORS)   # ลิสต์ของอุกกาบาต 4 ลูก
    ...
    def on_mouse_pressed(x, y):
        ...
        # ขั้นที่ 1: วาดจรวดใหม่ ณ ตำแหน่งที่คลิก
        new_rocket = draw_rocket(canvas, x, y)
        # ขั้นที่ 2: เก็บจรวดเข้า list rockets เพื่อให้ลูปขยับมัน
...
def draw_rocket(canvas, x, y):
    ...
    # สร้างลิสต์ว่างแล้วเพิ่มชิ้นส่วนทีละชิ้น เพื่อให้อ่านง่าย
    parts = []
    parts.append(body)
    parts.append(nose)
    parts.append(window)
    ...
    parts.append(flame)
    # คืนค่าลิสต์ทั้งหมด เพื่อให้ขยับและลบพร้อมกัน
    return parts
...
def create_meteors(canvas, count):
    ...
    for i in range(count):
        ...
        # ขั้นที่ 1: วาดอุกกาบาตที่ตำแหน่งสุ่มแล้วได้วัตถุกลับมา
        meteor = draw_meteor(canvas, x, y)
        # ขั้นที่ 2: รวมวัตถุกับความเร็วเป็น tuple เพื่อใช้ตอน animate
        meteor_data = (meteor, speed_x, speed_y)
        # ขั้นที่ 3: เก็บ tuple นี้เข้าลิสต์ meteors
...
def animate_meteors(canvas, meteors):
    ...
    for meteor_data in meteors:
        # แยกข้อมูลออกจาก tuple ให้เป็นตัวแปรแยกๆ
        ...
        # หาตำแหน่งปัจจุบันของอุกกาบาตเพื่อตรวจว่าหลุดจอหรือยัง
...
def animate_rockets(canvas, rockets):
    ...
    """
    # สร้างสำเนาของลิสต์ rockets ก่อน เพื่อให้ลบของจริงได้อย่างปลอดภัยระหว่างวนลูป
    rockets_copy = list(rockets)
    for parts in rockets_copy:
        # ขยับทุกชิ้นส่วนของจรวดขึ้นพร้อมกัน
        for part in parts:
            canvas.move(part, 0, -ROCKET_SPEED)
        # ตรวจสอบว่าตัวจรวด (ชิ้นแรกในลิสต์) หลุดออกขอบบนหรือยัง
        body = parts[0]
        ...
        if body_top_y < -ROCKET_BODY_HEIGHT:
            ...
            for part in parts:
                canvas.delete(part)
            # และเอาออกจากลิสต์ rockets ตัวจริง เพื่อไม่ต้องขยับซ้ำ
